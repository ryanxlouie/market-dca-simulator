...
import getDailyDataTQQQ from '../lib/mocks/getDailyDataTQQQ.json';
import getAdjustedDailyDataTQQQ from '../lib/mocks/getAdjustedDailyDataTQQQ.json';
...
export async function getAdjustedDailyData(ticker, startDate) {
  let result;
  if (config.routeSettings === 'Mock') {
    let res = getAdjustedDailyDataTQQQ;
    result = res['Time Series (Daily)'];
  }
  else {
    let res = await axios.get(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${ticker}&outputsize=full&apikey=${keys.avKey}`);
  ...
  for (let prop in result) {
    ...
    dailyArray.push({
      ...
      low: parseFloat(result[prop]['3. low']),
      close: parseFloat(result[prop]['4. close']),
      adjustedClose: parseFloat(result[prop]['5. adjusted close']),
      volume: parseFloat(result[prop]['6. volume']),
    });
    if (readableDate === startDate) {
      break;
    }
